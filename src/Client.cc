 ...
 * Last modified : 2019-10-10 15:35
 * Filename      : Client.cc
 * Description   : 
 **********************************************************/
...
Client::Client(EventLoop* loop, sockaddr_in& serverAddr, const std::string& name = "BaseClient"):
    loop_(loop),
...
Client::Client(EventLoop* loop, std::string ip, uint16_t port, const std::string& name = "BaseClient"):
    loop_(loop),
    connCallback_(defaultConnCallback),
    messCallback_(defaultMessCallback),
    name_(name),
...
Client::Client(EventLoop* loop, uint16_t port, const std::string& name = "BaseClient"):
    loop_(loop),
    connCallback_(defaultConnCallback),
    messCallback_(defaultMessCallback),
    name_(name),
...
void Client::newConnection(int sockfd)
{
    ...
    ConnectionPtr conn(loop, connName, localAddr, peerAddr, sockfd);
    conn->setConnCallback(connCallback_);
    conn->setMessCallback(messCallback_);
    conn->setWriteCompleteCallback(writeCompleteCallback_);
    conn->setCloseCompleteCallback(std::bind(&Client::removeConnection, this, _1));
    {
        MutexGuard lock(mutex_);
        connection_ = conn;
    }
